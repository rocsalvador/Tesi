...
Contour::Contour(const vector<Point<int>> &points)
{
    this->points = points;
}

void Contour::computeBorder(uint samples, uint mapSize)
{
    
}

Point<int> Contour::nextToTop(stack<Point<int>> &S)
{
    Point<int> p = S.top();
    S.pop();
    Point<int> res = S.top();
...
void Contour::swap(Point<int> &p1, Point<int> &p2)
{
    Point<int> temp = p1;
    p1 = p2;
    p2 = temp;
...
// 1 --> Clockwise
// 2 --> Counterclockwise
int Contour::orientation(Point<int> p, Point<int> q, Point<int> r)
{
...
bool Contour::compare(const Point<int> &p1, const Point<int> &p2)
{
...
void Contour::computeConvexHull()
{
    int n = points.size();
    if (n == 0)
        return;
...
vector<Point<int>> Contour::getContour()
{
    return contour;
}

double Contour::getPerimeter() const
{
    return perimeter;
}

double Contour::getSideLength(uint i) const
{
    return sideLength[i];
}

void Contour::setContour(const vector<Point<int>>& contour)
{
    this->contour = contour;

    sideLength = vector<double>(contour.size());
    perimeter = 0;
    for (uint i = 0; i < contour.size(); ++i)
    {
        double distance = Point<int>::euclidianDistance(contour[i], contour[(i + 1) % contour.size()]);
        perimeter += distance;
        sideLength[i] = distance;
    }
}

double Contour::angle(const Point<int> &p1, const Point<int> &p2, const Point<int> &p3)
{
...
vector<double> Contour::computeMVC(const Point<int> &point)
{
    int n = contour.size();
    if (n == 0)
